...
# Mở file student_info.json là file chưa thông tin sinh viên và cập nhật
with open("studen_info.json", "r", encoding="utf-8") as json_file:
    data_info = json.load(json_file)
# Lấy dữ liệu sinh viên ra các biến
...
#--Nhãn tên--
label_ten = tk.Label(label_frame1_info, text = "Họ và tên : {}".format(tensinhvien), background = COLOR_TEXT, fg = COLOR_TEXT_INFO, font = (FONT_MAIN, SIZE_BIG(screen_width, screen_height), 'bold'), bg= COLOR_LABEL).pack(pady= 5)
#--Avatar--
...
icon_previous = Image.open(r"icon\previous.png").resize((SIZE_BIG(screen_width, screen_height),SIZE_BIG(screen_width, screen_height)), Image.ADAPTIVE) # Ảnh
