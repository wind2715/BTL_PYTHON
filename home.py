...
def get_dates():
    current_day = dt.now()
    nam = current_day.year
    thang = current_day.month
    ngay = current_day.day
    res = f"{nam}-{thang}-{ngay}"
    return res
...
def update_json():
    ...
    for data_week in datas:
        ...
        content = f"Lịch học hôm nay của bạn" + "\n"
        for data_day in data_week["ds_thoi_khoa_bieu"]:
            ...
            if len(time_slots[begin_lesson - 1]) < 7:
                begin = int(time_slots[begin_lesson - 1][:1])
                tmp = begin + count_tiet
            else:
                begin = int(time_slots[begin_lesson - 1][:2])
                tmp = begin + count_tiet
            gio_hoc = f"{begin}h-{tmp}h"
            if data_day["ngay_hoc"][:10] == get_dates():
                content += f"{gio_hoc}: {data_day['ten_mon']} tại phòng {data_day['ma_phong'][:6]}" + "\n"
...
time_slots = ["7h-8h", "8h-9h", "9h-10h", "10h-11h", "11h-12h", "12h-13h", "13h-14h", "14h-15h", "15h-16h", "16h-17h",
              "17h-18h", "18h-19h", "19h-20h", "20h-21h"]  # Mảng khung giờ học
lession_slots = ["Tiết 1", "Tiết 2", "Tiết 3", "Tiết 4", "Tiết 5", "Tiết 6", "Tiết 7", "Tiết 8", "Tiết 9", "Tiết 10",
                 "Tiết 11", "Tiết 12", "Tiết 13", "Tiết 14"]  # Mảng tiết học
schedule = {time: {day: "" for day in days_of_week} for time in time_slots}  # dict chứa dữ liệu các ô khóa biểu
subject_entries = {(day, time_slot): None for day in days_of_week for time_slot in
                   time_slots}  # dict chứa các ô khóa biểu dạng Text Widget
...
label_ma = tk.Label(label_frame1_info, text="Mã sinh viên : {}".format(masinhvien), background=COLOR_TEXT,
                    fg=COLOR_TEXT_INFO, font=(FONT_MAIN, SIZE_NORMAL(screen_width, screen_height),),
                    bg=COLOR_LABEL).pack(pady=5)
label_lop = tk.Label(label_frame1_info, text="Lớp : {}".format(lop), background=COLOR_TEXT, fg=COLOR_TEXT_INFO,
                     font=(FONT_MAIN, SIZE_NORMAL(screen_width, screen_height),), bg=COLOR_LABEL).pack(pady=5)
label_gioitinh = tk.Label(label_frame1_info, text="Giới tính : {}".format(gioitinh), background=COLOR_TEXT,
                          fg=COLOR_TEXT_INFO, font=(FONT_MAIN, SIZE_NORMAL(screen_width, screen_height),),
                          bg=COLOR_LABEL).pack(pady=5)
label_nganh = tk.Label(label_frame1_info, text="Ngành : {}".format(nganh), background=COLOR_TEXT, fg=COLOR_TEXT_INFO,
                       font=(FONT_MAIN, SIZE_NORMAL(screen_width, screen_height),), bg=COLOR_LABEL).pack(pady=5)

# Nút dailynote
daily_note = tk.Button(rounded_frame1_label, text="Daily Note", borderwidth=1, command=daily, relief="solid",
                       background=COLOR_MAIN,
                       fg='white', font=(FONT_MAIN, SIZE_NORMAL(screen_width, screen_height)))
...
update_button = tk.Button(rounded_frame1_label, text="Cập nhật Thời Khóa Biểu", command=update_schedule_from_json,
                          borderwidth=1, relief="solid", background=COLOR_MAIN, fg='white',
                          font=(FONT_MAIN, SIZE_NORMAL(screen_width, screen_height)))
update_button.place(relx=0.5, rely=0.83, anchor='s')

# Nút đăng xuất
button_signout = tk.Button(rounded_frame1_label, text="Đăng xuất", borderwidth=1, command=logout, relief="solid",
                           background=COLOR_MAIN,
                           fg='white', font=(FONT_MAIN, SIZE_NORMAL(screen_width, screen_height)))
...
icon_previous = Image.open(r"images\home_screen\icon\previous.png").resize(
    (SIZE_BIG(screen_width, screen_height), SIZE_BIG(screen_width, screen_height)), Image.ADAPTIVE)  # Ảnh
...
tk.Button(frame3, image=photo_previous, command=get_previous_value,
          font=(FONT_MAIN, SIZE_NORMAL(screen_width, screen_height), 'bold'), bg=COLOR_MAIN, fg=COLOR_TEXT,
          borderwidth=0, relief='solid', highlightcolor='white').grid(row=0, column=0, sticky=tk.W + tk.E + tk.S + tk.N)
tk.Button(frame3, image=photo_next, command=get_next_value,
          font=(FONT_MAIN, SIZE_NORMAL(screen_width, screen_height), 'bold'), bg=COLOR_MAIN, fg=COLOR_TEXT,
          borderwidth=0, relief='solid', highlightcolor='white').grid(row=0, column=len(days_of_week) + 1,
                                                                      sticky=tk.W + tk.E + tk.S + tk.N)

# Hiển thị cột khung thời gian
for j, time_slot in enumerate(time_slots):
    tk.Label(frame3, text=time_slot, font=(FONT_MAIN, SIZE_NORMAL(screen_width, screen_height), 'bold'), bg=COLOR_MAIN,
             fg=COLOR_TEXT).grid(row=j + 1, column=0, sticky=tk.W + tk.E + tk.S + tk.N)

# Hiển thị hàng thứ
for j, lession_slot in enumerate(lession_slots):
    tk.Label(frame3, text=lession_slot, font=(FONT_MAIN, SIZE_NORMAL(screen_width, screen_height), 'bold'),
             bg=COLOR_MAIN, fg=COLOR_TEXT).grid(row=j + 1, column=len(days_of_week) + 1,
                                                sticky=tk.W + tk.E + tk.S + tk.N)

# Đưa các ô khóa biểu dạng Text widget vào mảng đã khai báo trước đó
for i, day in enumerate(days_of_week):
    tk.Label(frame3, text=day, font=(FONT_MAIN, SIZE_NORMAL(screen_width, screen_height), 'bold'), bg=COLOR_MAIN,
             fg=COLOR_TEXT).grid(row=0, column=i + 1, sticky=tk.W + tk.E + tk.S + tk.N)
    for j, time_slot in enumerate(time_slots):
        text_widget = tk.Text(frame3, height=2, width=40, font=(FONT_MAIN, SIZE_SMALL(screen_width, screen_height) - 1),
                              state=tk.DISABLED)
...
# Bo góc frame 4
rounded_frame4_image = create_rounded_frame(int(frame2_width * (8 / 9)), int(49 * (screen_height / 1000)), 15,
                                            COLOR_LABEL)
...
subject_label = tk.Label(label_frame4_info, text="Ghi chú", bg=COLOR_MAIN,
                         font=(FONT_MAIN, SIZE_SMALL(screen_width, screen_height), 'bold'), fg=COLOR_TEXT, width=10)
subject_label.pack(side=tk.LEFT, padx=5)

subject_entry = tk.Entry(label_frame4_info, borderwidth=1, relief='solid',
                         font=(FONT_MAIN, SIZE_SMALL(screen_width, screen_height)))
subject_entry.pack(side=tk.LEFT, padx=5)

time_label = tk.Label(label_frame4_info, text="Thời gian", bg=COLOR_MAIN,
                      font=(FONT_MAIN, SIZE_SMALL(screen_width, screen_height), 'bold'), fg=COLOR_TEXT, width=10)
...
day_label = tk.Label(label_frame4_info, text="Thứ", bg=COLOR_MAIN,
                     font=(FONT_MAIN, SIZE_SMALL(screen_width, screen_height), 'bold'), fg=COLOR_TEXT, width=5)
...
add_button = tk.Button(label_frame4_info, text="Thêm Ghi Chú", bg=COLOR_MAIN,
                       font=(FONT_MAIN, SIZE_SMALL(screen_width, screen_height), 'bold'), fg=COLOR_TEXT,
                       command=lambda: update_schedule_to_data_private(day_combobox.get(), subject_entry.get(),
                                                                       time_combobox.get()))
...
# Bo góc frame5
rounded_frame5_image = create_rounded_frame(int(frame2_width * (5 / 9)), int(49 * (screen_height / 1000)), 15,
                                            COLOR_LABEL)
...
combobox_weeks = ttk.Combobox(frame5, width=50, height=10, font=(FONT_MAIN, SIZE_SMALL(screen_width, screen_height)),
